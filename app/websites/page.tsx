      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 dark:text-gray-500">
            {search ? "No websites match your search." : "No websites saved yet. Sign in to start adding websites!"}
          </p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div key={website.id} className="relative">
              {editingId === website.id ? (
                <form onSubmit={handleEdit} className="card p-5 space-y-3">
                  ...
                  <textarea value={editData.description} onChange={(e) => setEditData({ ...editData, description: e.target.value })} className="textarea-field text-sm" placeholder="Description" rows={2} required />
                  <input value={editData.url} onChange={(e) => setEditData({ ...editData, url: e.target.value })} className="input-field text-sm" placeholder="URL" required />
                ...
                <div className="card-interactive p-5">
                  ...
                  {/* Always-visible edit/delete controls when signed in */}
                  {user && (
        ...
        <div className="mt-8">
          {showForm ? (
            <form onSubmit={handleAdd} className="card p-6 max-w-lg mx-auto space-y-4">
              <h3 className="font-semibold text-gray-900 dark:text-white">Add a Website</h3>
              <div>
